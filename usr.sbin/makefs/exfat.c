...
/* makefs */
static int exfat_populate(struct exfat *, const char *, fsnode *, fsnode *,
	fsinfo_t *, const bool);
...
/* see gpl/github.com/relan/exfat/mkfs/mkexfatfs.8 */
void
...
exfat_init_image(const char *image, fsinfo_t *fsopts)
{
	...
	if (fsopts->sparse) {
		printf("%s: ftruncate(%d, 0x%lx)\n", __func__, fd, size);
		if (ftruncate(fd, size) == -1)
			err(1, "ftruncate failed");
	} else {
		printf("%s: posix_fallocate(%d, 0, 0x%lx)\n",
			__func__, fd, size);
...
exfat_makefs(const char *image, const char *dir, fsnode *root, fsinfo_t *fsopts)
{
	...
	printf("%s: image=\"%s\" directory=\"%s\"\n", __func__, image, dir);
	printf("%s: create_size=0x%lx\n", __func__, exfat_opts->create_size);
	printf("%s: volume_label=%s\n", __func__, exfat_opts->volume_label);
	printf("%s: volume_serial=0x%x\n", __func__, exfat_opts->volume_serial);
	printf("%s: first_sector=%ld\n", __func__, exfat_opts->first_sector);
	printf("%s: spc_bits=%d\n", __func__, exfat_opts->spc_bits);
	...
	bool verbose = (debug & DEBUG_FS_POPULATE) ? true : false;
	if (exfat_populate(&ef, dir, root, root, fsopts, verbose) == -1)
		errx(1, "exFAT populate %s failed", image);
	exfat_unmount(&ef);
	TIMER_RESULTS(start, "exfat_populate");
	if (verbose)
		printf("\n");
	print_line();
...
static void
exfat_show(const char *f, const char *p, bool isdir, bool verbose)
{
	if (debug & DEBUG_FS_POPULATE) {
		if (0) {
			printf("XXX %s %s from %s\n",
				isdir ? "DIR" : "REG", p, f);
		} else {
			printf("%c", isdir ? 'd' : '.');
			fflush(stdout);
...
static int
exfat_populate(struct exfat *ef, const char *path, fsnode *root, fsnode *parent,
	fsinfo_t *fsopts, const bool verbose)
{
	fsnode *cur;
	...
	for (cur = root->next; cur != NULL; cur = cur->next) {
		...
		int ret = snprintf(f, sizeof(f), "%s/%s", path, cur->name);
		...
		if (S_ISDIR(cur->type)) {
			assert(cur->child);
			exfat_show(f, p, true, verbose);

			ret = exfat_mkdir(ef, p);
			if (ret < 0)
				errx(1, "exfat_mkdir(\"%s\") failed: %s",
					p, strerror(-ret));
			if (exfat_populate(ef, f, cur->child, cur, fsopts,
				verbose) == -1)
				errx(1, "%s: %s", __func__, f);
			continue;
		}
